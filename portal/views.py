...
def portal_main_page(request):
	...
	try:
		...
		context_dict={}
		room=hostel_name.g_floor+hostel_name.f_floor+hostel_name.s_floor 
		context_dict['room'] = room
		context_dict['loop_times'] = [i+1 for i in range(room)]
		...
		try:        
			notifications=Rooms.objects.get(Q(roll_no_1=request.user)|Q(roll_no_2=request.user))
