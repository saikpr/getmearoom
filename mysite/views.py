...
def pass_recovery(request):
	...
	return HttpResponse("ewerwewrewrew")
...
def room_page(request):
	...
	if request.method == 'GET':
		...
		if pref1.isdigit()==False and pref2.isdigit()==False:
			return HttpResponse("Please enter a valid number")	
		total_room=Hostel.objects.get(hostel_name=hostel)
		room_sum=total_room.g_floor+total_room.f_floor+total_room.s_floor			
	...
	if pref1 and pref2 :
		...
		if int(pref1)>0 and int(pref2)>0 and int(pref1)<50 and int(pref2)<50:
			...
			elif '2 Available' in pref2_data:
				try:
					student=Student.objects.get(roll_no=request.user)
					student.room_alloted=int(pref2)
					student.save()
					return HttpResponse("Room "+ pref2+ " is alloted to you")
				except:
					return HttpResponse("Something went wrong.Try again")	

			elif '1 Available' in pref1_data:
				try:
										
					roll_list=pref1_data.split()
					roll_no_reciever=roll_list[0]
					roll_no_reciever=roll_no_reciever.strip()							
					try:					
						check=Notifications.objects.get(roll_no_sender=request.user,roll_no_reciever=roll_no_reciever)
						return HttpResponse("Request Already sent")
					except:
						pass						
					x=Notifications(roll_no_sender=Student.objects.get(roll_no=request.user),roll_no_reciever=Student.objects.get(roll_no=roll_no_reciever))
					x.full_clean()
					x.save()
					return HttpResponse("Request has been sent to" +roll_no_reciever+" for approval for room no. "+pref1)
				except:
					return HttpResponse("Something went wrong.Try again")

			elif '1 Available' in pref2_data:
				try:
					roll_list=pref2_data.split()
					roll_no_reciever=roll_list[0]
					roll_no_reciever=roll_no_reciever.strip()						
					try:					
						check=Notifications.objects.get(roll_no_sender=request.user,roll_no_reciever=roll_no_reciever)
					...
					return HttpResponse("Request has been sent to" +roll_no_reciever2+" for approval for room no. "+pref2)
		...
		#	else:
		#		return HttpResponse("Enter room numbers less than or equal to 50")			
		
			
		#elif now<room_start_30:
		
		#elif now<room_start_45:

		#elif now<room_start_60:	
